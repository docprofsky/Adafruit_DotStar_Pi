/*------------------------------------------------------------------------
  ...
  Modified by Schuyler St. Leger to use libmraa.
...
mraa_gpio_context clockPin = NULL;
mraa_gpio_context dataPin = NULL;
...
static PyObject *begin(DotStarObject *self) {
    if(self->dataPin == 0xFF) { // Use hardware SPI
        ...
        // TODO: implement better GPIO initialization
        // if(gpio == NULL) { // First time accessing GPIO?
        clockPin = mraa_gpio_init(self->clockPin);
        dataPin = mraa_gpio_init(self->dataPin);

        // Make sure pins were properly initialized
        if(clockPin == NULL || dataPin == NULL) {
            return NULL;
        }

        mraa_gpio_use_mmaped(clockPin, 1);
        mraa_gpio_use_mmaped(dataPin, 1);
        // }

        // Set 2 pins as outputs.
        mraa_gpio_dir(clockPin, MRAA_GPIO_OUT);
        mraa_gpio_dir(dataPin, MRAA_GPIO_OUT);
        // data+clock LOW
        mraa_gpio_write(clockPin, 0);
        mraa_gpio_write(dataPin, 0);
    }
    Py_INCREF(Py_None);
    return Py_None;
}
    ...
    // } else if(self->dataMask) { // Bitbang
        ...
        while(len--) {
            ...
            for(bit = 0x80; bit; bit >>= 1) {
                if(byte & bit) mraa_gpio_write(dataPin, 1);
                else           mraa_gpio_write(dataPin, 0);
                clockPulse();
            }
        }
        mraa_gpio_write(dataPin, 0);
